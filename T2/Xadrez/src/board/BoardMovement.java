...
public class BoardMovement
{
	...
	public boolean moveFrom(Coordinate From, int line, char turn)
	{
		...
		{	
			...
			{
				throw new WrongTurnException("\tNao é o seu turno!");
			}
	...
	public boolean moveTo(Coordinate To, int line, char turn)
	{
		...
		try
		{
			...
			ClientSocketSingleton.getInstance().getMessager().sendMessage("MOVE:: " + from.getX() + ", " + from.getY() + ", " + to.getX() + ", " + to.getY());
			if(check() == turn)
			{
			...
			if( check() != 'n' ) 
			{
				...
				if( mate(turn) )
				{
					...
					GamePresenter.getInstance().showFinalAlert("Check Mate!");
				}
				System.out.printf("!\n");
			}
		}
