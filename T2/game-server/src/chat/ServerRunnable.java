...
public class ServerRunnable implements Runnable{
	...
	public String readAndAct(String msg) {
		...
		if (part1.compareTo("REGISTER") == 0) {
			...
			Player user = ServerSingleton.getInstance().getUsersHash().get(username);
			user.getOpponent().getOutStream().println(msg);
			user.renewLastPlayed();
			user.getOpponent().renewLastPlayed();
			return username + " made move " + part2;
		} else if (part1.compareTo("###") == 0){
			Player opponent = ServerSingleton.getInstance().getUsersHash().get(username).getOpponent();
			if(opponent != null){
				ServerSingleton.getInstance().endgame(username, opponent.getUsername());
			} else {
				ServerSingleton.getInstance().removeClient(client, username, out);
			}
